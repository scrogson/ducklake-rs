...
use crate::error::{DuckPondError, Result};
use serde::{Deserialize, Serialize};

/// Configuration for DuckPond catalog database and data storage
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DuckPondConfig {
...
impl DuckPondConfig {
    ...
    pub fn from_env() -> Result<Self> {
        let database_url = std::env::var("DATABASE_URL").map_err(|_| DuckPondError::Config {
    ...
    pub fn validate(&self) -> Result<()> {
        if self.database_url.is_empty() {
            return Err(DuckPondError::Config {
    ...
    pub fn examples() -> Vec<(&'static str, &'static str, &'static str)> {
        vec![
            (
                "PostgreSQL",
                "postgresql://user:pass@localhost/duckpond",
                "For production use with full ACID guarantees",
            ),
            (
                "MySQL",
                "mysql://user:pass@localhost/duckpond",
